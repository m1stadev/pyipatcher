...
def get_type(data):  # from kairos
    if BIT_RANGE(data, 24, 28) == 0x10 and data >> 31:
        ...
        (((data >> 22) | 0x100) == 0x3E1 and ((data >> 10) % 4))
        or ((data >> 22 | 0x100) == 0x3E5)
        or ((data >> 23) == 0x18)
    ):
        return 'ldr'
        ...
        BIT_RANGE(data, 21, 31) == 0x1C2
        or BIT_RANGE(data, 22, 31) == 0xE5
        or ((BIT_RANGE(data, 21, 31) == 0x1C3 and BIT_RANGE(data, 10, 11) == 0x2))
    ):
        return 'ldrb'
        ...
        (BIT_RANGE(data, 21, 30) == 0x359)
        or (BIT_RANGE(data, 24, 30) == 0x6B)
        or (BIT_RANGE(data, 24, 30) == 0x71)
    ):
        return 'subs'
...
def subtype(opcode, type):
    if type == 'ldrh':
        ...
        if ((opcode >> 22) | (1 << 8)) == 0b1111100001 and BIT_RANGE(
            opcode, 10, 11
        ) == 2:
            return 'reg'
        elif (opcode >> 31) or (
            BIT_RANGE(opcode | SET_BITS(1, 30), 22, 31) == 0b1111100101
        ):
            return 'imm'
        else:
            return 'literal'
        ...
        if (
            BIT_RANGE(opcode, 21, 31) == 0b00111000011
            and BIT_RANGE(opcode, 10, 11) == 2
        ):
            return 'reg'
        else:
            return 'imm'
        ...
        if (
            BIT_RANGE(opcode, 21, 31) == 0b00111000001
            and BIT_RANGE(opcode, 10, 11) == 2
        ):
            return 'reg'
        else:
...
def imm(pc, opcode, type) -> int:
    ...
    if type == 'adr':
        return pc + signExtend(
            (BIT_RANGE(opcode, 5, 23) << 2) | (BIT_RANGE(opcode, 29, 30)), 21
        )
    elif type == 'b':
        return pc + ((opcode % (1 << 26)) << 2)
    elif type == 'adrp':
        return ((pc >> 12) << 12) + signExtend(
            ctypes.c_uint64(
                (((((opcode % (1 << 24)) >> 5) << 2) | BIT_RANGE(opcode, 29, 30)))
            ).value
            << 12,
            32,
        )
        ...
        return (ctypes.c_uint64(BIT_RANGE(opcode, 5, 20)).value) << (
            BIT_RANGE(opcode, 21, 22) * 16
        )
    elif type == 'ldr':
        if st == 'st_immediate':
